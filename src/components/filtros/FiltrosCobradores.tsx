...
import { FiltrosCobradorState } from '../../types/cobrador';


interface FiltrosCobradoresProps {
    filtros: FiltrosCobradorState;
    totalCobradores: number;
    onCambiarFiltro: (campo: keyof FiltrosCobradorState, valor: unknown) => void;
    onLimpiarFiltros: () => void;
}

const FiltrosCobradores: React.FC<FiltrosCobradoresProps> = ({
    filtros,
    totalCobradores,
    onCambiarFiltro,
    onLimpiarFiltros
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <button
                    ...
                    className={`
                        ...
                        transition-colors
                        ${mostrarFiltros
                            ? 'border-red-600 bg-red-100 text-red-700 hover:bg-red-200 hover:text-red-800 dark:border-red-500 dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-900/40'
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                            <div className="space-y-1">
                                <p className="text-sm text-gray-600 dark:text-gray-400">
                                    <span className="font-semibold text-gray-900 dark:text-white">{totalCobradores}</span> cobradores encontrados
                                </p>
                                {filtrosActivos > 0 && (
                                    <div className="flex flex-wrap gap-2">
                                        {filtros.estado !== 'todos' && (
                                        ...
                                        {filtros.ordenNombre && (
                                            <span className="inline-flex items-center gap-1 rounded-full bg-purple-50 px-2 py-0.5 text-xs font-medium text-purple-700 dark:bg-purple-900/30 dark:text-purple-400">
                                                Orden: Nombre {filtros.ordenNombre === 'asc' ? '↑' : '↓'}
                                            </span>
                                        )}
                                    </div>
                                )}
...
export default FiltrosCobradores;